...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let isSignIn =  UserDefaults.standard.value(forKey: "UserIsLog") as? Bool ?? false
        
        let vc = isSignIn ? ProfileVC() : SignInVC()
        let navigationViewController = UINavigationController(rootViewController: vc)
        self.navigtionController = navigationViewController
        self.navigtionController?.navigationBar.isHidden = true
